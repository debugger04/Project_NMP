<?php
	...
	if ($_POST['id_user']) {
		...
		$sql = "SELECT ifnull(sum(p.harga * c.qty), 0) as grand_total, ifnull(sum(c.qty),0) as qty FROM cart c inner join produk p on p.id = c.produk_id where c.users_id =".$_POST['id_user'];
		$result = $c->query($sql);
		$arr_data = array();
