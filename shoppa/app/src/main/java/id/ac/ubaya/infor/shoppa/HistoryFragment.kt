...
class HistoryFragment : Fragment() {
            ...
            Response.Listener {
                ...
                if(obj.getString("result") == "OK") {
                    ...
                    for(i in 0 until data.length()) {
                        val transObj = data.getJSONObject(i)
                        val trans = Transaction(
                            transObj.getString("id"),
                            transObj.getString("tgl_order"),
                            transObj.getInt("grandtotal"),
                            transObj.getString("status")
                        )
                        transactions.add(trans)
                    }
                    updateList()
                    Log.d("list_trans", transactions.toString())
                }
            },
    ...
    fun updateList() {
        ...
        var recyclerView = v?.findViewById<RecyclerView>(R.id.recyclerViewHistory)
        recyclerView?.layoutManager = lm
        recyclerView?.setHasFixedSize(true)
        recyclerView?.adapter = HistoryAdapter(transactions, this)
    }
